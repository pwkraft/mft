...
The article proposes a weighting method to improve conventional dictionary approaches in order to capture the relative emphasis on each moral dimension (while correcting for ubiquitous terms and overall response length). Figure~\ref{fig:mft_weights} displays the weights used for each dictionary term that is mentioned at least once (terms that never appear are omitted). Terms that are very common, like ``care'', or ``foreign'', receive comparatively low weights. Such common words are more likely to be used in multiple contexts and cannot be uniquely ascribed to the moral domain, while %\footnote{E.g., the statement ``I mostly care about foreign relations.'' should not be viewed as a moral argument.} 
...
\input{../calc/tab/tobit_virtue}
\input{../calc/tab/tobit_vice}
\clearpage
\item \textit{by question type (like vs. dislike)}
\input{../calc/tab/tobit_like}
\input{../calc/tab/tobit_dislike}
\clearpage
\item \textit{by party/candidate (Democrat vs. Republican)}
\input{../calc/tab/tobit_dem}
\input{../calc/tab/tobit_rep}
\clearpage
\item \textit{by party (in- vs. out-party)}
\input{../calc/tab/tobit_in}
\input{../calc/tab/tobit_out}
\end{enumerate}
