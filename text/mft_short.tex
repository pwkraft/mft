...
Figure~\ref{fig:tobit_media} presents the results of a Tobit model where effects are again decomposed into the probability of mentioning any moral foundation (left panel) as well as the emphasis on morality, given that any foundation was mentioned (right panel). Individuals who are exposed to media sources that report on the campaign in a more moralized manner put a stronger emphasis on moral considerations in their open-ended responses. Thus, citizens either adopt moral reasoning from their media environment or actively seek out sources that are consistent with their moral views.
% However, ascertaining the causal ordering of this relationship has to be left for future research. 
