...
\end{equation}
where $\text{MFT}_{if}$ denotes the score of document $i$ for foundation $f$, $W_i$ is the total number of words in document $i$, $t$ indicates a term in the set of signal terms of foundation dictionary $\mathcal{D}_f$, and $w_{it}$ denotes the number of occurrences of term $t$ in document $i$. In the case of the analyses presented here, each document is the verbatim response of an individual to a set of open-ended questions. As such, an individual's MFT score for each foundation is the overall proportion of words in the response that signal the respective foundation.
