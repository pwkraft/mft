...
anes2008 <- anes2008[anes2008$spanish != 1 & anes2008$wc != 0,]
...
prop_plot(data=list(anes2008)
          ...
          , file = "fig/prop_ideol_2008.pdf", width = 3, height = 3, lim = c(0,.7))
...
ggsave(filename = "fig/tobit_ideol_2008.pdf", width = 5, height = 2.5)
...
m7_res <- rbind(sim(m7, iv=data.frame(harm_s = min(anes2008$harm_s)+c(0,1)), robust=T)
                , sim(m7, iv=data.frame(fairness_s = min(anes2008$fairness_s)+c(0,1)), robust=T)
                , sim(m7, iv=data.frame(ingroup_s = min(anes2008$ingroup_s)+c(0,1)), robust=T)
                , sim(m7, iv=data.frame(authority_s = min(anes2008$authority_s)+c(0,1)), robust=T))
...
ggsave(filename = "fig/ols_feel_2008.pdf", width = 5, height = 3)
...
## simulation of predicted probabilities / first differences
m8_res <- rbind(sim(m8, iv=data.frame(harm_s = min(anes2008$harm_s)+c(0,1)))
                , sim(m8, iv=data.frame(fairness_s = min(anes2008$fairness_s)+c(0,1)))
                , sim(m8, iv=data.frame(ingroup_s = min(anes2008$ingroup_s)+c(0,1)))
                , sim(m8, iv=data.frame(authority_s = min(anes2008$authority_s)+c(0,1))))
...
ggplot(m8_res, aes(x = mean, y = var+.1-.2*(cond=="Yes"), col=cond, shape=cond)) +
  ...
  theme(legend.position="bottom", legend.box="horizontal") +
  scale_color_grey(start=0,end=.5)
ggsave(filename = "fig/logit_vote_2008.pdf", width = 3, height = 3)
...
## model estimation
m3 <- list(NULL)
m3[[1]] <- vglm(general_s ~ polknow + relig + educ + age + female + black + lwc
                , tobit(Lower = 0), data=anes2008)
m3[[2]] <- vglm(general_s ~ polmedia + relig + educ + age + female + black + lwc
                , tobit(Lower = 0), data=anes2008)
m3[[3]] <- vglm(general_s ~ poldisc + relig + educ + age + female + black + lwc
                , tobit(Lower = 0), data=anes2008)
m3[[6]] <- vglm(general_s ~ polknow + polmedia + poldisc
                + relig + educ + age + female + black + lwc
                , tobit(Lower = 0), data=anes2008)
lapply(m3, summary)

## simulation of predicted probabilities / first differences
m3_res <- rbind(sim(m3[[1]], iv=data.frame(polknow=range(anes2008$polknow, na.rm = T)))
                , sim(m3[[2]], iv=data.frame(polmedia=range(anes2008$polmedia, na.rm = T)))
                , sim(m3[[3]], iv=data.frame(poldisc=range(anes2008$poldisc, na.rm = T)))
                , sim(m3[[6]], iv=data.frame(polknow=range(anes2008$polknow, na.rm = T)))
                , sim(m3[[6]], iv=data.frame(polmedia=range(anes2008$polmedia, na.rm = T)))
...
ggsave(filename = "fig/tobit_learn_2008.pdf", width = 5, height = 3)
...
m4_know[[1]] <- vglm(harm_s ~ ideol*polknow_c + relig + educ + age + female + black + 
                       lwc, tobit(Lower = 0), data=anes2008)
...
m4_all[[1]] <- vglm(harm_s ~ ideol*polknow_c + ideol*polmedia_c + ideol*poldisc_c + 
                    relig + educ + age + female + black + lwc, tobit(Lower = 0), data=anes2008)
...
m4_res <- rbind(sim(models = m4_know
                    , iv=data.frame(polknow_c=rep(range(anes2008$polknow_c, na.rm = T),each=2)
                                    , ideolModerate = rep(0,4)
                                    , ideolConservative = c(0,1,0,1)))
                , sim(models = m4_media
                      , iv=data.frame(polmedia_c=rep(range(anes2008$polmedia_c, na.rm = T),each=2)
                                      , ideolModerate = rep(0,4)
                                      , ideolConservative = c(0,1,0,1)))
                , sim(models = m4_disc
                      , iv=data.frame(poldisc_c=rep(range(anes2008$poldisc_c, na.rm = T),each=2)
                                      , ideolModerate = rep(0,4)
                                      , ideolConservative = c(0,1,0,1)))
                , sim(models = m4_all
                      , iv=data.frame(polknow_c=rep(range(anes2008$polknow_c, na.rm = T),each=2)
...
ggplot(m4_res, aes(x = mean, y = var+.1-.2*(cond=="Yes"), col=cond, shape=cond)) +
  ...
  scale_color_grey(start=0,end=.5)
ggsave(filename = "fig/tobit_learnideol_2008.pdf", width = 4, height = 6)
